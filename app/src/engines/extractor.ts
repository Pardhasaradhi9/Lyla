import { initLlama, LlamaContext } from 'llama.rn';
import { EXTRACTOR_SYSTEM_PROMPT } from '@/prompts/extractor-prompt';

const EXTRACTOR_CONFIG = {
  n_ctx: 1024,
  n_gpu_layers: 99,
  n_batch: 256,
  use_mlock: true,
  use_mmap: true,
  cache_type_k: 'q8_0',
  cache_type_v: 'q8_0',
  max_tokens: 256,
  temperature: 0.1,
  top_k: 10,
  penalty_repeat: 1.05,
} as const;

export const extractorEngine = {
  isLoaded: false,
  context: null as LlamaContext | null,

  async init(modelPath: string): Promise<void> {
    if (this.context) {
      await this.release();
    }

    try {
      this.context = await initLlama({
        model: modelPath,
        use_mlock: EXTRACTOR_CONFIG.use_mlock,
        n_ctx: EXTRACTOR_CONFIG.n_ctx,
        n_gpu_layers: EXTRACTOR_CONFIG.n_gpu_layers,
      });
      this.isLoaded = true;
    } catch (error) {
      console.error('[ExtractorEngine] Failed to load:', error);
      throw error;
    }
  },

  async extractFacts(
    userMessage: string,
    assistantResponse: string,
  ): Promise<Array<{ fact: string; category: string; entity: string | null }>> {
    if (!this.context) return [];

    const prompt = `<|im_start|>system\n${EXTRACTOR_SYSTEM_PROMPT}<|im_end|>\n<|im_start|>user\nUser said: "${userMessage}"\nAssistant replied: "${assistantResponse}"<|im_end|>\n<|im_start|>assistant\n`;

    try {
      const result = await this.context.completion(
        {
          prompt,
          n_predict: EXTRACTOR_CONFIG.max_tokens,
          temperature: EXTRACTOR_CONFIG.temperature,
          top_k: EXTRACTOR_CONFIG.top_k,
          penalty_repeat: EXTRACTOR_CONFIG.penalty_repeat,
          stop: ['<|im_end|>', '<|im_start|>'],
        },
        () => {},
      );

      const text = result.text.trim();
      return parseExtractorOutput(text);
    } catch (error) {
      console.error('[ExtractorEngine] Extraction error:', error);
      return [];
    }
  },

  async release(): Promise<void> {
    if (this.context) {
      await this.context.release();
      this.context = null;
    }
    this.isLoaded = false;
  },
};

function parseExtractorOutput(
  text: string,
): Array<{ fact: string; category: string; entity: string | null }> {
  const jsonMatch = text.match(/\[[\s\S]*\]/);
  if (!jsonMatch) return [];

  try {
    const parsed = JSON.parse(jsonMatch[0]);
    if (!Array.isArray(parsed)) return [];

    return parsed
      .filter(
        (item: unknown) =>
          typeof item === 'object' &&
          item !== null &&
          'fact' in (item as Record<string, unknown>) &&
          'category' in (item as Record<string, unknown>),
      )
      .map((item: Record<string, unknown>) => ({
        fact: String(item.fact),
        category: String(item.category),
        entity: item.entity ? String(item.entity) : null,
      }));
  } catch {
    return [];
  }
}
